...
from batch_sampler import FewShotBatchSamplerModified, StratifiedFewShotBatchSamplerBenignCycled
...
split = "ss_11_2d_no"                                                                           # version of data split
...
road_type = f"fab_{split}_split_80test5val"                                                     # The pre-splitted dataset name
if (DATASET == 0):
...
if (EPISODIC_TRAIN):
    N_WAY, K_support, K_query = 5, 5, 10
    train_fs_sampler = StratifiedFewShotBatchSamplerBenignCycled(
        torch.tensor(tr_dataset.y), 
        N_way=N_WAY, 
        K_support=K_support, 
        K_query=K_query
    )
...
val_ds = RoadDataset_v3(dataset=DATASET, mode='val', road_type=road_type, with_time=True, embed=d_model, seperate_proto=TEST_SHOT > 0, shot = TEST_SHOT, new=new, ws = ws, s = s)
...
if not new and DATASET == 1:
    mar_type = f"mar_{split}_split_80test"
